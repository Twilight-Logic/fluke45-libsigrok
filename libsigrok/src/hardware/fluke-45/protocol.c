...
SR_PRIV int fl45_get_status(const struct sr_dev_inst *sdi, struct sr_datafeed_analog *analog, int idx)
{
	/* Gets the current state of the meter and sets analog object parameters */
...
SR_PRIV int fl45_get_modifiers(const struct sr_dev_inst *sdi, struct sr_datafeed_analog *analog, int idx)
{
	struct dev_context *devc;
//	struct sr_scpi_dev_inst *scpi;
...
int get_reading_dd(char *reading, size_t size)
{
	...
	if (errno != 0) return 2;
	exp = exp*(-1); // Negative exponent increses digits, a positive one reduces
...
//	scpi = sdi->conn;
	...
	for (i=0; i<devc->num_channels; i++) {
		...
		if (analog[i].meaning->mq != 0 && channel->enabled) {
			...
			if (i==0) res = fl45_scpi_get_response(sdi, "VAL1?");
			if (i==1) res = fl45_scpi_get_response(sdi, "VAL2?");
			...
			if (strcmp(reading, "+1E+9") == 0) {
				fv = INFINITY;
				sr_dbg("Reading OL (infinity): %s", devc->response);
...
SR_PRIV int fl45_scpi_get_response(const struct sr_dev_inst *sdi, char *cmd)
{
	...
	/* Attempt to get an scpi reponse */
	if (sr_scpi_get_string(sdi->conn, cmd, &devc->response) != SR_OK)
		return SR_ERR;
//    sr_dbg("RAW response: %s", devc->response);
	/* Deal with RS232 '=>' prompt */
	if (strcmp(devc->response, "=>") == 0){
		/* If the response is a prompt then ignore and read the next response in the buffer */
		devc->response = NULL;
	...
	if (strcmp(devc->response, "!>") == 0 || (strcmp(devc->response, "?>") == 0)){
		/* Unable to execute CMD */
		devc->response = NULL;
	}
//	sr_dbg("Modified response: %s", devc->response);
